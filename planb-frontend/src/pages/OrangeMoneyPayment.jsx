...
import { 
  ...
  Smartphone, CreditCard, AlertCircle, Crown, Sparkles, Star, RefreshCw, QrCode
} from 'lucide-react';
...
// Plans avec réductions (à partir de 9 mois)
const PRICING_PLANS = {
...
/**
 * Page de paiement Orange Money (Style glassmorphism)
 */
export default function OrangeMoneyPayment() {
  ...
  // Récupérer selectedMonths depuis l'URL ou utiliser 1 par défaut
  const [selectedMonths, setSelectedMonths] = useState(() => {
    if (isRenewal) return 1;
    return parseInt(searchParams.get('months')) || 1;
  });

  // Synchroniser selectedMonths avec l'URL au montage initial
  useEffect(() => {
  ...
  const handlePayment = async () => {
    ...
    if (!cleanPhone || cleanPhone.length < 9) {
      toast.error('Veuillez entrer un numéro de téléphone Orange Money valide');
      return;
    }
    ...
    try {
      // Utiliser l'API backend pour créer le paiement Orange Money
      ...
      if (data.success && data.payment) {
        ...
        sessionStorage.setItem('orange_payment_id', data.payment.id);
        sessionStorage.setItem('orange_phone', cleanPhone);
        sessionStorage.setItem('orange_months', selectedMonths.toString());
        sessionStorage.setItem('orange_transaction_id', data.payment.transactionId || '');

        // Si QR Code disponible, l'afficher
        if (data.payment.qrCode) {
          setQrCode(data.payment.qrCode);
          setPaymentStatus('pending_confirmation');
          toast.success('Scannez le QR Code avec votre application Orange Money');
        } else if (data.payment.paymentUrl) {
          // Rediriger vers Orange Money
          toast.success(
            `Redirection vers Orange Money...\nMontant: ${planPrice.toLocaleString()} FCFA`, 
            { duration: 3000 }
          );
          ...
          // Mode USSD ou confirmation par téléphone
          toast.info(
            `Demande de paiement envoyée\nVeuillez confirmer sur votre téléphone Orange Money`, 
            { duration: 5000 }
          );
          setPaymentStatus('pending_confirmation');
        }
      ...
      toast.error(error.response?.data?.error || error.message || 'Échec du paiement');
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-blue-50 to-purple-50 relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute w-96 h-96 bg-orange-300/20 rounded-full blur-3xl -top-48 -left-48 animate-pulse" />
      ...
      >
        <div className="space-y-6 pb-24 relative z-10">
          ...
          >
            <div className="w-20 h-20 bg-gradient-to-br from-orange-400 to-orange-600 rounded-full flex items-center justify-center mx-auto mb-4 shadow-xl">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-blue-700 mt-1">
                    Votre nouvel abonnement démarrera automatiquement après confirmation du paiement.
                  </p>
                </div>
              </div>
            </motion.div>
          ...
          <AnimatePresence mode="wait">
            {paymentStatus === 'idle' || paymentStatus === 'error' ? (
              ...
              >
                ...
                <GlassCard className="bg-white/60 backdrop-blur-xl border-white/20 shadow-2xl">
                  ...
                  <div className="space-y-4">
                    <Input
                      label="Numéro de téléphone Orange Money"
                      type="tel"
                    ...
                    {/* Infos Orange Money */}
                    <div className="bg-orange-50/80 backdrop-blur-sm p-3 rounded-xl">
                      <div className="flex gap-2 text-sm text-orange-800">
                        <AlertCircle size={16} className="flex-shrink-0 mt-0.5" />
                        <div>
                          <p className="font-medium mb-1">Comment ça marche ?</p>
                          <ol className="space-y-1 text-xs">
                            <li>1. Entrez votre numéro Orange Money ci-dessus</li>
                            <li>2. Vous recevrez une demande de paiement sur votre téléphone</li>
                            <li>3. Confirmez le paiement avec votre code PIN Orange Money</li>
                            <li>4. Votre compte PRO sera activé automatiquement</li>
                          </ol>
                        </div>
                      </div>
                    </div>
            ...
            ) : paymentStatus === 'processing' || paymentStatus === 'pending_confirmation' ? (
              <motion.div
                key="processing"
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
              ...
              >
                <GlassCard className="bg-white/60 backdrop-blur-xl border-white/20 text-center">
                  {qrCode ? (
                    <>
                      <QrCode size={64} className="mx-auto mb-4 text-orange-500" />
                      <h3 className="text-xl font-bold text-secondary-900 mb-2">
                        Scannez le QR Code
                      </h3>
                      <p className="text-secondary-600 mb-4">
                        Utilisez votre application Orange Money pour scanner ce code
                      </p>
                      <div className="bg-white p-4 rounded-xl mb-4 inline-block">
                        <img src={qrCode} alt="QR Code Orange Money" className="w-48 h-48" />
                      </div>
                    </>
                  ) : (
                    <>
                      ...
                      <p className="text-secondary-600 mb-4">
                        Veuillez confirmer le paiement sur votre téléphone Orange Money
                      </p>
                      <div className="bg-orange-50/80 backdrop-blur-sm p-3 rounded-xl">
                        <p className="text-sm text-orange-800">
                          ⏱️ En attente de confirmation Orange Money
                        </p>
                      </div>
                    </>
                  )}
        ...
        {(paymentStatus === 'idle' || paymentStatus === 'error') && (
          <div className="pb-6">
            <div className="max-w-md mx-auto space-y-3">
              ...
              <div className="bg-gradient-to-r from-orange-500 to-orange-600 rounded-2xl p-4 text-center text-white shadow-lg">
              ...
              <Button
                ...
                icon={<Shield size={20} />}
              >
                {loading ? 'Traitement...' : 'Payer avec Orange Money'}
              </Button>
