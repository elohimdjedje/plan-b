...
import { ArrowLeft, Check, Camera, X, Home, Car, Palmtree, Building2, DoorClosed, Trees, Store, Bike, Hotel, Lightbulb, DollarSign, Phone, MessageCircle, Mail, Globe } from 'lucide-react';
...
export default function Publish() {
  ...
  const { accountType = 'FREE', user } = useAuthStore();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Upload visite virtuelle si disponible (PRO uniquement)
      if (formData.virtualTourFile && listingId && (accountType === 'PRO' || user?.isLifetimePro)) {
        try {
          await virtualTourAPI.upload(listingId, formData.virtualTourFile);
          ...
          >
            ...
            {step === 3 && (
              <GlassCard>
                ...
                {/* Visite Virtuelle 360° - PRO uniquement pour upload, tutoriel accessible à tous */}
                <div className="mt-6 pt-6 border-t border-secondary-200">
                  {(accountType === 'PRO' || user?.isLifetimePro) ? (
                    <>
                      <h3 className="text-lg font-semibold mb-3 flex items-center gap-2 text-secondary-900">
                        <Globe size={20} className="text-primary-600" />
                        Visite Virtuelle 360° (Optionnel)
                      </h3>
                      <p className="text-sm text-secondary-600 mb-4">
                        Ajoutez une photo panoramique 360° pour permettre aux acheteurs de visiter virtuellement votre bien.
                      </p>
                      
                      <div className="border-2 border-dashed border-secondary-300 rounded-xl p-6 text-center">
                        {formData.virtualTourFile ? (
                          <div className="space-y-4">
                            <div className="relative">
                              <img 
                                src={URL.createObjectURL(formData.virtualTourFile)} 
                                alt="Preview visite virtuelle"
                                className="w-full h-48 object-cover rounded-lg"
                              />
                              <button
                                onClick={() => handleChange('virtualTourFile', null)}
                            ...
                            <p className="text-sm text-secondary-600">
                              {formData.virtualTourFile.name} ({(formData.virtualTourFile.size / 1024 / 1024).toFixed(2)} MB)
                            </p>
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  if (file) {
                                    ...
                                    handleChange('virtualTourFile', file);
                                  }
                                }}
                                className="hidden"
                              />
                    ...
                    <div className="bg-gradient-to-r from-yellow-50 to-orange-50 border border-yellow-200 rounded-xl p-4 mb-4">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          <h3 className="font-semibold text-yellow-900 mb-1">Visite Virtuelle 360°</h3>
                          <p className="text-sm text-yellow-700 mb-2">
                            Cette fonctionnalité est disponible uniquement pour les comptes PRO.
                          </p>
                          <Button
                            variant="primary"
