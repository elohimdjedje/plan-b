...
export default function WavePayment() {
  ...
  const handlePayment = async () => {
    ...
    try {
      // Utiliser l'API backend pour créer le paiement Wave
      ...
      if (data.success && data.payment?.paymentUrl) {
        ...
        sessionStorage.setItem('wave_payment_id', data.payment.id);
        sessionStorage.setItem('wave_phone', cleanPhone);
        sessionStorage.setItem('wave_months', selectedMonths.toString());
        sessionStorage.setItem('wave_transaction_id', data.payment.transactionId || '');
        ...
        // Fallback: Utiliser le lien personnel si l'API n'est pas configurée
        console.warn('API Wave non configurée, utilisation du lien personnel');
        const wavePaymentLink = 'https://pay.wave.com/m/M_ci_cCLTAMUtr2FQ/c/ci/';
        
        toast.info(
          `Mode manuel activé\nMontant à payer: ${planPrice.toLocaleString()} FCFA\nVous devrez entrer le montant manuellement sur Wave`, 
          { duration: 5000 }
        );
        
        sessionStorage.setItem('wave_phone', cleanPhone);
        sessionStorage.setItem('wave_months', selectedMonths.toString());
        sessionStorage.setItem('wave_amount', planPrice.toString());
        
        setTimeout(() => {
          window.location.href = wavePaymentLink;
        }, 2000);
      }
