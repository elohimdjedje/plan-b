 ...
 * - Envoi au backend pour persister les vues
 */

const VIEW_STORAGE_KEY = 'planb_viewed_listings';
const FINGERPRINT_KEY = 'planb_device_fingerprint';
const MIN_VIEW_DURATION = 3000; // 3 secondes minimum sur la page
const VIEW_EXPIRY_HOURS = 24; // Une vue par 24h

/**
 * Génère un fingerprint unique pour identifier l'appareil
 * Combinaison de plusieurs facteurs pour une identification fiable
 */
function generateFingerprint() {
  ...
  // Générer un nouveau fingerprint basé sur les caractéristiques du navigateur
  const components = [
    navigator.userAgent,
    navigator.language,
    ...
    navigator.platform,
    // Ajouter un identifiant aléatoire pour renforcer l'unicité
...
export function trackListingView(listingId, onViewCounted = null) {
  ...
  const viewTimer = setTimeout(async () => {
    ...
    if (timeSpent >= MIN_VIEW_DURATION) {
      ...
      // Envoyer la vue au backend avec fingerprint anti-fraude
      try {
        const fingerprint = getDeviceFingerprint();
        const result = await listingsAPI.incrementViews(listingId, fingerprint);
        console.log(`Vue envoyée pour l'annonce ${listingId}:`, result.counted ? 'comptée' : 'déjà vue');
      } catch (error) {
        console.error(`Erreur envoi vue au backend:`, error);
      }
