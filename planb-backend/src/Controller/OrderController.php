...
class OrderController extends AbstractController
{
    ...
    public function createOrder(Request $request): JsonResponse
    {
        ...
        // Générer le lien de paiement selon le moyen choisi
        try {
            if ($data['payment_method'] === 'wave') {
                ...
                // Sauvegarder le session ID Wave
                $order->setWaveSessionId($result['session_id']);
                $this->entityManager->flush();
                ...
                return $this->json([
                    ...
                    'order_id' => $order->getId(),
                    'payment_method' => 'wave',
                    'payment_link' => $result['wave_launch_url'],
             ...
             * 
            elseif ($data['payment_method'] === 'orange_money') {
                $result = $this->orangeMoneyService->generatePaymentLink(
                    (float) $data['amount'],
                    (string) $order->getId(),
                    $client->getPhone()
                );
                ...
                // Sauvegarder le payment token Orange Money
                $order->setOmPaymentToken($result['payment_token']);
                $this->entityManager->flush();
                ...
                return $this->json([
                    ...
                    'validity_minutes' => $result['validity']
                ], 201);
            }
            */
        ...
        return $this->json(['error' => 'Méthode de paiement non supportée'], 400);
    }
    ...
    public function waveCallback(int $orderId, Request $request): JsonResponse
    {
        ...
        if (!$order) {
            $this->logger->error('Wave callback: order not found', ['order_id' => $orderId]);
        ...
        $this->logger->info('Wave callback received', [
            'order_id' => $orderId,
            'params' => $queryParams,
        ...
        if ($order->getWaveSessionId()) {
            try {
                $status = $this->waveService->getTransactionStatus($order->getWaveSessionId());
                
                $this->logger->info('Wave transaction status', [
                    'order_id' => $orderId,
                    'status' => $status
                ]);

                // Mettre à jour la commande
                $order->setApiStatus($status['status']);
                $order->setApiTransactionId($status['id']);
                $order->setApiCode($status['status']);
                ...
                $this->logger->error('Error checking Wave status', [
                    'order_id' => $orderId,
                    'error' => $e->getMessage()
                ]);
            }
