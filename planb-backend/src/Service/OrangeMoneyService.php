...
class OrangeMoneyService
{
     ...
     * @param string $signature Signature HTTP header X-Orange-Signature
     * @return bool
     */
    public function verifyWebhook(string $payload, string $signature): bool
    {
        $webhookSecret = $_ENV['OM_WEBHOOK_SECRET'] ?? '';
        
        if (empty($webhookSecret)) {
            $this->logger->warning('Orange Money webhook secret not configured');
            // En développement, accepter si pas de secret configuré
            if ($_ENV['APP_ENV'] === 'dev') {
                return true;
            }
            return false;
        ...
        // Orange Money utilise généralement HMAC-SHA256
